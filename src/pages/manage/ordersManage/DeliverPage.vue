...
const handleSingle = (scope, option) => {//点击单行操作按钮
  if (option == 'deliver') {
    store.getOrdersDeliver({ ids: [scope.row.orderId] })
  } else {
...
const handleMore = (option) => { //点击批量操作
  if (ifOperate.value && multipleTableRef.value.getSelectionRows().length) {
    if (option == 'deliver') {
      store.getOrdersDeliver({ ids: multipleTableRef.value.getSelectionRows().map(item => { return item.orderId }) })
    } else {
...
<template>
  ...
  <div class="row1">
    ...
    <el-button v-show="ifOperate" @click="handleMore('prepare')" type='success'>{{ '发货' }}</el-button>
  ...
  <div ref="tableContainerRef" class="row2">
    <el-table ref="multipleTableRef" stripe border :height="currentHeight" style="width:100%;text-align: center;"
      ...
      <!-- 订单号列 -->
      <el-table-column label="ID" min-width="200px" property="orderNo" />
      <!-- 订单内容列 -->
      ...
      <el-table-column label="操作" fixed="right" min-width="100px">
        <template #default="scope">
          <el-button :disabled="ifOperate || scope.row.orderStatus == -1 ? true : false" size="default"
            @click="handleSingle(scope, 'deliver')" type="success">发货</el-button>
