...
let recommend = reactive({})//编辑场景下通知子组件本次修改的是哪条
...
const handleEdit = (scope) => {//点击编辑
  drawer.value = true
  ifEdit.value = true
  recommend.goodsId = scope.row.goodsId
  recommend.configRank = scope.row.configRank
  recommend.configName = scope.row.configName
  recommend.configId = scope.row.configId
}
const handleDelete = (scope) => {//点击单行删除
  store.getRecommendDelete({ ids: [scope.row.configId] })
}
const deleteMore = () => {//点击批量删除
  if (ifOperate.value && multipleTableRef.value.getSelectionRows().length) { store.getRecommendDelete({ ids: multipleTableRef.value.getSelectionRows().map(item => { return item.configId }) }) }
...
watchEffect(async () => {
  await store.getRecommendList(currentPage.value, pageSize.value)
})
...
<template>
  ...
  <div ref="tableContainerRef" class="row2">
    <el-table ref="multipleTableRef" stripe border :height="currentHeight" style="width:100%;text-align: center;"
      :data="store.recommendList" row-key="configId">
      <!-- 勾选列，仅点击批量管理时出现 -->
      <el-table-column v-if="ifOperate" type="selection" min-width="55px" reserve-selection fixed />
      <!-- 序号列 -->
      <el-table-column type="index" min-width="70px" label="序号" />
      <!-- 秒杀ID列 -->
      <el-table-column label="ID" min-width="70px" property="configId"> </el-table-column>
      <!-- 秒杀名称列 -->
      <el-table-column label="推荐名称" min-width="100px" property="configName"> </el-table-column>
      <!-- 图片列 -->
  ...
  <!-- 编辑框 -->
  <RecommendDrawer v-model:drawer="drawer" @updata:drawer="drawer = $event" :ifEdit="ifEdit" :recommend="recommend">
  </RecommendDrawer>
</template>
