...
watchEffect(() => {
  if (props.ifEdit) {//编辑时表单带入初始数据
    ruleForm.configRank = props.recommend.configRank,
      ruleForm.goodsId = props.recommend.goodsId,
      ruleForm.configName = props.recommend.configName,
      ruleForm.configId = props.recommend.configId//编辑场景下需要额外提交的数据
  } else {//新增时表单清空
    ruleForm.configRank = '',
      ruleForm.goodsId = '',
      ruleForm.configName = '',
...
const rules = reactive({//定义表单校验规则
  ...
  configName: [{ required: true, message: '请设置推荐名称', trigger: 'blur' }],
})
const submitForm = (ruleFormRef) => {//点击提交新增
  ...
  ruleFormRef.validate(async (valid) => {
    if (valid) {
      await store.getRecommendAdd({ configRank: ruleForm.configRank, goodsId: ruleForm.goodsId, configName: ruleForm.configName })
...
const submitEdit = (ruleFormRef) => {//点击提交修改
  ...
  ruleFormRef.validate(async (valid) => {
    if (valid) {
      await store.getRecommendEdit({ configRank: ruleForm.configRank, goodsId: ruleForm.goodsId, configName: ruleForm.configName, configId: ruleForm.configId })
      console.log(ruleFormRef)
      resetForm(ruleFormRef)//提交成功后重置表单
...
<template>
  <el-drawer v-model="drawer" direction="rtl" :lock-scroll="false" size="50%" destroy-on-close>
    <!-- 标题 -->
    <template #header>
      <h4>{{ `${props.ifEdit ? '编辑' : '新增'}推荐商品配置` }}</h4>
    </template>
    <!-- 表单 -->
    <template #default>
      <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="80px" class="demo-ruleForm">
        ...
        <el-form-item label="选择商品" prop="goodsId">
          <!-- 商品选择器 -->
          <el-select v-model="ruleForm.goodsId" class="m-2" placeholder="请选择商品" size="large" :validate-event="false"
            :popper-append-to-body="false">
            <el-option-group>
              <el-input v-model="filter" class="w-50 m-2" placeholder="搜索现有商品" :prefix-icon="Search" @change="onFilter"
                style="width:80%;margin-left:30px;margin-bottom: 20px;" />
              <!-- 分页器 -->
              <el-pagination v-model:currentPage="currentPage" small layout="total, prev, pager, next, jumper"
                  ...
                  <el-card :body-style="{ padding: '10px', width: '300px', height: '140px' }"
                    ...
                    <el-row style="flex:1">
                      <el-col :span="8" style="display:flex;justify-content: center;align-items: center;">
                        <el-image :src="`http://192.168.1.3:28019${item?.goodsCoverImg}`"
        ...
        <el-form-item label="推荐名称" prop="configName">
